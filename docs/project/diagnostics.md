...
## 2026-03-15 — Castle base-space calibration transform check (diagnostic)
- Runtime now uses contain/full-frame base rendering (`Math.min(...)`) and stores `currentCastleTransform` from the rendered base image (`sourceWidth/sourceHeight`, `renderedWidth/renderedHeight`, `scale`, `centerX/centerY`).
- Slot center placement currently honors base-space calibration: slot `x/y` (or offset-adjusted `x/y`) are normalized by `layout.baseWidth/baseHeight` and mapped into the rendered base rect, so coordinate placement is not using raw scene pixels.
